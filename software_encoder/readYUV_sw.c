...
// Reads an 8x8 block from the RGB buffer, converts to YCbCr, and fills the arrays
void read_444_format_sw(JPEG_ENCODER_STRUCTURE *jpeg, UINT8 *input_ptr, INT16 *Y1, INT16 *CB, INT16 *CR, int mcu_col, int mcu_row) {
    int i, j;
    int width = jpeg->cols;
    int height = jpeg->rows;
    
    for (i = 0; i < 8; i++) {
        for (j = 0; j < 8; j++) {
            // Handle edge padding by clamping coordinates to max width/height
